...
def main(args):
  ...
  with lb.Uninterrupt() as u:
    for x, y in recycle(train_loader):
      ...
      summary.add_scalar('train_loss', c_num, cnt)
      summary.add_scalar('lr', lr, cnt)
      cnt += 1
      ...
      if accum_steps == args.batch_split:
        ...
        if args.eval_every and step % args.eval_every == 0:
          all_loss, all_top1, all_top5 = run_eval(model, valid_loader, device, chrono, logger, step)
          #print('step {} loss {}'.format(step, np.mean(all_loss)))
          #print('step {} loss {}'.format(step, np.mean(all_top1)))
          #print('step {} loss {}'.format(step, np.mean(all_top5)))
          summary.add_scalar('val_loss', np.mean(all_loss), step)
          summary.add_scalar('val_top1', np.mean(all_top1), step)
          summary.add_scalar('val_top5', np.mean(all_top5), step)
